...
from ...log_creator import get_file_logger, get_console_logger
...
class ModelServerClient:
    ...
    def create_openai_embeddings(
        ...
        Example:
            ...
            >>> log(len(embeddings))  # 2
            >>> log(len(embeddings[0]))  # 1536 (embedding dimension)
        """
...
if __name__ == "__main__":
    ...
    try:
        ...
        console_logger.info("Example 1: Creating embeddings")
        console_logger.info("=" * 50)
        texts = ["Hello world", "How are you?", "Machine learning is amazing"]
        embeddings = client.create_openai_embeddings(texts)
        console_logger.info(f"Created {len(embeddings)} embeddings")
        console_logger.info(f"Embedding dimension: {len(embeddings[0])}")
        console_logger.info(f"First embedding (first 5 values): {embeddings[0][:5]}")

        # Example 2: Chat completion
        console_logger.info("\n" + "=" * 50)
        console_logger.info("Example 2: Chat completion with OpenAI")
        console_logger.info("=" * 50)
        messages = [
            {"role": "system", "content": "You are a helpful assistant."},
            {"role": "user", "content": "What is the capital of France?"},
        ...
        console_logger.info(f"Response: {response}")

        # Example 3: Sentence transformers
        console_logger.info("\n" + "=" * 50)
        console_logger.info("Example 3: Sentence transformers")
        console_logger.info("=" * 50)
        ...
        console_logger.info(f"Created {len(sent_embeddings)} sentence embeddings")
        console_logger.info(f"Embedding dimension: {len(sent_embeddings[0])}")  # type: ignore

        # Example 4: Claude completion
        console_logger.info("\n" + "=" * 50)
        console_logger.info("Example 4: Claude completion")
        console_logger.info("=" * 50)
        claude_messages = [
            {
                "role": "user",
        ...
        console_logger.info(f"Claude response: {claude_response}")
