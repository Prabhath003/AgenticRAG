...
# src/infrastructure/utils/json_parser.py
import ast
import json
import re
import subprocess
...
def extract_json_from_llm_response(response_text: str) -> Dict[str, Any]:
    ...
    for strategy_name, parse_function in parsing_strategies:
        try:
            ...
            if result is not None and isinstance(result, dict):  # type: ignore
...
def _try_progressive_fix_parse(json_str: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        result: Dict[str, Any] = json.loads(current_json)
...
def _fix_quotes_comprehensively(json_str: str) -> str:
    ...
    for line in lines:
        ...
        def fix_array_elements(match: re.Match[str]):
...
if __name__ == "__main__":
    ...
    logger.info("=== LLM JSON Parser Test Results ===\n")

    for i, test_case in enumerate(test_cases, 1):
        logger.info(f"Test Case {i}:")
        logger.info("Input:", repr(test_case.strip()[:100]) + "...")
        ...
        try:
            ...
            if result:
                ...
                logger.error("❌ FAILED - returned empty dict")
        except Exception as e:
    ...
    logger.info("\n=== Summary ===")
    logger.info("Parser handles:")
    logger.info("• Comments (// # /* */)")
    logger.info("• Unquoted keys and values")
    logger.info("• Missing brackets and braces")
    logger.info("• Trailing commas")
    logger.info("• Mixed quote styles")
    logger.info("• JavaScript object syntax")
    logger.info("• Progressive error recovery")
