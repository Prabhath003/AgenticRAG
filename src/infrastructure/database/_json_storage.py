...
class JSONStorage:
    ...
    def _load_all_shards(self, collection_name: str) -> Dict[str, Dict[str, Any]]:
    ...
    def find(
        ...
        for _, doc in collection.items():
            if query is None or self._matches_query(doc, query):
                if projection:
                    doc = self.apply_projection(doc, projection)
                results.append(doc)
    ...
    def _extract_shard_key(self, query: Dict[str, Any]) -> Optional[str]:
        ...
        if "entity_ids" in query:
            entity_ids: Union[List[str], str] = query["entity_ids"]
            if isinstance(entity_ids, str):
                return entity_ids
            elif isinstance(entity_ids, list) and len(entity_ids) == 1:  # type: ignore
                return entity_ids[0]
    ...
    def update_one(
        ...
        with lock:
            ...
            collection: Optional[Dict[str, Dict[str, Any]]] = self._read_json(file_path)
    ...
    def _extract_shard_key_from_update(self, update: Dict[str, Any]) -> Optional[str]:
        ...
        if "$set" in update:
            ...
            if "entity_ids" in update["$set"]:
                entity_ids: List[str] = update["$set"]["entity_ids"]
                if isinstance(entity_ids, list) and len(entity_ids) > 0:  # type: ignore
                    return entity_ids[0]
    ...
    def _matches_query(
        self, doc: Dict[str, Any], query: Dict[str, Union[List[Dict[str, Any]], Dict[str, Any]]]
    ) -> bool:
        ...
        for key, value in query.items():
            if key.startswith("$"):
                ...
                if key == "$or":
                    if not any(self._matches_query(doc, q) for q in value):  # type: ignore
                        return False
                elif key == "$and":
                    if not all(self._matches_query(doc, q) for q in value):  # type: ignore
                        return False
            else:
                # Handle nested fields with dot notation
                doc_value = self.get_nested_value(doc, key)
                ...
                if isinstance(value, dict):
                    ...
                    for op, op_value in value.items():
                        if op == "$exists":
                            ...
                            for not_op, not_op_value in op_value.items():  # type: ignore
                                if not_op == "$regex":
                                    if doc_value is not None and isinstance(doc_value, str):
                                        if re.search(not_op_value, doc_value):  # type: ignore
                                            return False
                                elif not_op == "$eq":
    ...
    def get_nested_value(self, doc: Dict[str, Any], key: str) -> Any:
    ...
    def _apply_update(self, doc: Dict[str, Any], update: Dict[str, Any]) -> bool:
        ...
        if "$inc" in update:
            for key, increment_value in update["$inc"].items():
                current_value = self.get_nested_value(doc, key)
    ...
    def apply_projection(self, doc: Dict[str, Any], projection: Dict[str, int]) -> Dict[str, Any]:
    ...
    def _group_stage(
        ...
        group_spec: Dict[str, Union[str, Dict[str, Union[str, Dict[str, Any]]]]],
    ) -> List[Dict[str, Any]]:
        ...
        for doc in docs:
            ...
            if isinstance(group_key, str) and group_key.startswith("$"):
                key_value = self.get_nested_value(doc, group_key[1:])
            else:
                key_value = group_key
            ...
            for field, op in group_spec.items():
                ...
                if isinstance(op, dict):  # type: ignore
                            ...
                            if isinstance(op_field, dict):
                                ...
                                for k, v in op_field.items():
                                    if isinstance(v, str) and v.startswith("$"):
                                        push_doc[k] = self.get_nested_value(doc, v[1:])
                                    else:
                                        push_doc[k] = v
                                groups[key_value][field].append(push_doc)
                            elif isinstance(op_field, str) and op_field.startswith("$"):  # type: ignore
                                # Push a field value
                                groups[key_value][field].append(
                                    self.get_nested_value(doc, op_field[1:])
                                )
...
class JSONCollection:
    ...
    def find_one(
        ...
        if result and projection:
            result = self.storage.apply_projection(result, projection)
        return result
    ...
    def sort(self, key: str, direction: int = 1):
        ...
        class SortableCursor:
            def __init__(self, collection: JSONCollection, key: str, direction: int):
            ...
            def __iter__(self):
                ...
                try:
                    _ = self.key.split(".")
                    results.sort(
                        key=lambda x: self.collection.storage.get_nested_value(x, self.key) or "",
