...
from ._custom_types import ResponseResponse, Utterance, ResponseRequiredRequest
from ...config import Config
from ...log_creator import get_file_logger
from .._rag_system import get_rag_system
...
# def call_openai(payload: Dict[str, Any], api_key: str=Config.AZURE_OPENAI_KEY or "", initial_wait: float = 60.0):
#     """
#     Call Azure OpenAI Chat API with exponential backoff retry on rate limiting.
#     Retries indefinitely until successful when encountering rate limit (429) errors.
...
#     deployment = payload.get('model', 'gpt-4.1-nano')
...
#             print(f"Rate limited (429). Attempt {attempt + 1}. Waiting {wait_time:.1f}s before retry...")
...
class ResearchAgent:
    ...
    def __init__(
        ...
        id: str,
        entity_name: str,
        use_entity_scoped: bool = True,
        on_stage_change_callback: Optional[Callable[..., Any]] = None,
        ...
        if (
            ...
            try:
                ...
                logger.info(
                    f"Initialized ResearchAgent with Azure OpenAI (deployment: {self.model_name})"
                )
        ...
        if use_entity_scoped:
            ...
            logger.info(
                f"Initialized ResearchAgent with entity-scoped RAG for {entity_name} (ID: {id})"
            )
        else:
    ...
    def parse_citations_from_content(
        self, content: str
    ...
    def _get_model_pricing(self, model_name: str) -> Tuple[float, float, float]:
        ...
        pricing_map: Dict[str, Tuple[float, float, float]] = {
            ...
            "gpt-4o-mini": (
                ...
                0.60,
                0.075,
            ),  # $0.15/1M input, $0.60/1M output, $0.015/1M cache read
    ...
    def _calculate_openai_cost(
        ...
        """
        input_token_price, output_token_price, cache_token_price = self._get_model_pricing(
            self.model_name
        )
    ...
    async def research_question(
        ...
        relationship_ids: Optional[List[Dict[str, str]]] = None,
        ...
        try:
            ...
            async for chunk in stream:
                ...
                if chunk.usage:
                    ...
                    yield ResponseResponse(
                        ...
                        node_ids=node_ids,
                        relationship_ids=[
                            f"{rel['source_id']}:{rel['target_id']}" for rel in relationship_ids
                        ],
                        cited_node_ids=[],
                        citations=[],
                ...
                if chunk.choices and len(chunk.choices) > 0:
                    ...
                    if delta.content:
                        ...
                        response = ResponseResponse(
                            ...
                            estimated_cost_usd=sum(
                                s.estimated_cost_usd for s in cumulative_services
                            ),
                        ...
                        messages = self.add_to_messages(
                            messages, Utterance(role="agent", content=content_buffer)
                        )
                        yield response
                    ...
                    if (
                        ...
                        try:
                            ...
                            result, node_ids, relationship_ids, cumulative_services = (
                            ...
                            if isinstance(result, dict):
                                if result.get("action") == "end_call":
                                    logger.info(
                                        f"Tool-based end call with message: {result['message']}"
                                    )
                            ...
                            async for response in self.research_question(
                                ...
                                recursive_cumulative_services = [
                                    Service.from_dict(service) for service in response.services_used
                                ]
                                yield response
                            ...
                            logger.error(
                                f"Error parsing function arguments: {func_arguments} - {e}"
                            )
                            ...
                            logger.error(
                                f"Error executing function {func_call['func_name']}: {e}",
                                exc_info=True,
                            )
                            openai_cost = self._calculate_openai_cost(
                                total_input_tokens, total_output_tokens
                        ...
                        _, cited_node_ids, citations = self.parse_citations_from_content(
                            content_buffer
                        )
            ...
            if not final_usage_received and total_output_tokens == 0:
                logger.warning(
                    f"No usage information received from OpenAI API. Using token estimation."
                )
                # For error responses, estimate conservatively
                total_output_tokens = self._estimate_tokens_from_content(
                    "I apologize, but I encountered an issue. How else can I help you with finding courses?"
                )
                # Estimate input tokens from conversation history
                for msg in messages:
                    if isinstance(msg, dict) and msg.get("content"):
                        total_input_tokens += self._estimate_tokens_from_content(
                            str(msg.get("content", ""))
                        )
            ...
            messages = self.add_to_messages(
                ...
                Utterance(
                    role="agent",
                    content="I apologize, but I encountered an issue. How else can I help you?",
                ),
            )
            yield response
    ...
    def _generate_system_prompt(self) -> str:
        ...
        performance_note = (
            " (OPTIMIZED: Using entity-scoped RAG for 10-100x faster search!)"
...
Use the tools strategically and extensively to provide thorough, well-researched, data-driven answers based on the company's documents."""
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "k": {
                                "type": "integer",
                                "description": "Number of results to return (default: 25, max recommended: 25)",
                                "default": 25,
                            },
                            "message": {
                                "type": "string",
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "doc_id": {
                                "type": "string",
                                "description": "Document ID from previous search result (doc_id is not doc name do list documents to get the doc_id) (format: doc_xxxxx)",
                            },
                            "chunk_order_index": {
                                "type": "integer",
                                "description": "Current chunk index from previous search result - will return chunk at index-1",
                            },
                            "message": {
                                "type": "string",
                                "description": "message to user to hint about the ongoing analysis",
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            ...
                            "chunk_order_index": {
                                "type": "integer",
                                "description": "Target chunk index from previous search result - will return surrounding chunks",
                            },
                            "context_size": {
                                "type": "integer",
                                "description": "Number of chunks before/after to include (1=±1 chunk, 2=±2 chunks for broader context). Use 2 for complex topics.",
                                "default": 1,
                            },
                            "message": {
                                "type": "string",
                ...
                "function": {
                    ...
                    "parameters": {
                        ...
                        "properties": {
                            "doc_id": {
                                "type": "string",
                                "description": "Document ID to read sequentially (doc_id is not doc name do list documents to get the doc_id) (format: doc_xxxxx from previous searches or document list)",
                            },
                            "message": {
                                "type": "string",
                                "description": "message to user to hint about the ongoing analysis",
    ...
    async def execute_function(
        ...
        cumulative_services: Optional[List[Service]] = None,
    ):
        ...
        try:
            ...
            if func_name == "semantic_search_within_entity":
                ...
                if not results:
                    return (
                        f"No results found for query '{query}' in documents.",
                        node_ids,
                        relationship_ids,
                        cumulative_services,
                    )
                ...
                if result:
                    ...
                    return (
                        f"No previous chunk found for {doc_id}:{chunk_idx}",
                        node_ids,
                        relationship_ids,
                        cumulative_services,
                    )
                ...
                if result:
                    ...
                    return (
                        f"No next chunk found for {doc_id}:{chunk_idx}",
                        node_ids,
                        relationship_ids,
                        cumulative_services,
                    )
                ...
                response = (
                    f"**Context around {doc_id}:{chunk_idx}:\n{json.dumps(result, indent=1)}**\n\n"
                )
                ...
                if not results:
                    return (
                        f"No documents found for {self.entity_name}",
                        node_ids,
                        relationship_ids,
                        cumulative_services,
                    )
                ...
                for i, chunk in enumerate(results[:10]):
                    ...
                    if i > 0:
                        prev_idx = (
                            results[i - 1].get("chunk", {}).get("chunk_order_index", "unknown")
                        )
                    ...
                    response += (
                        f"**Chunk {idx}:** {content[:200]}{'...' if len(content) > 200 else ''}\n\n"
                    )
