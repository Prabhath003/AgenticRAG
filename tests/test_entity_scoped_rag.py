...
from src.core._rag_system import (
...
def test_entity_scoped_rag():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = index_document_entity_scoped(
            ...
            file=File(os.path.basename(doc1_path), content),
        ...
        results = index_documents_parallel(
            {"company_123": [doc2_path], "company_456": [doc3_path], "company_789": [doc4_path]}
        )

        elapsed = time.time() - start_time

        print(
            f"✓ Indexed {sum(len(docs) for docs in results.values())} documents in {elapsed:.2f}s"
        )
        ...
        search_results = search_entity_scoped(
            entity_id="company_123", query="financial performance revenue", k=3
        )
        ...
        start_time = time.time()
        sequential_results: List[Document] = []
